...
const answers = {
  GENERAL: {
    ...
    2: "Use unit testing (Jest, Mocha), integration tests, browser dev tools, and linters like ESLint or Prettier.",
    3: "Use CSS resets, test on multiple browsers, and use feature detection (e.g., Modernizr).",
    ...
    15: "Unit tests test individual parts. Integration tests check how parts work together.",
  ...
  GIT: {
    ...
    32: "1. Clone: `git clone URL` <br> 2. Create branch  <br> 3. Push <br> 4. Open PR on GitHub.",
    33: "Specifies which files/folders Git should ignore (e.g., node_modules, .env).",
    34: "Use `git revert <commit>` or `git reset` followed by a force push (with caution).",
    35: ".ENV stores secrets. If not ignored, secrets may be exposed publicly."
  },
  HTML: {
    36: "`article` = self-contained content, `aside` = side info, `nav` = navigation.",
    37: "A tag that does not need closing",
    38: "Block = full width (e.g., `div`), Inline = inside text flow (e.g., `span`).",
    39: "Use `a` for links/navigation; `button' for actions like submitting forms.",
    40: "Use semantic HTML, alt text, ARIA roles, and ensure keyboard navigation.",
  ...
  CSS: {
    ...
    48: "Relative = based on parent; Absolute = based on nearest relative parent; Fixed = viewport; Sticky = scroll-aware.",
  ...
  JavaScript: {
    53: "`let` = block-scoped, mutable; `const` = block-scoped, immutable; `var` = function-scoped, older.",
    54: "Bubbling = inner to outer, Capturing = outer to inner. Use `addEventListener` with capture flag.",
  ...
  "AI Development and Theory": {
    ...
    67: "Logistic Regression for spam detection or binary classification.",
    68: "Linear Regression for predicting house prices based on size.",
    ...
    74: "Transformers use attention mechanisms to understand token relationships. Core of models like BERT, GPT.",
